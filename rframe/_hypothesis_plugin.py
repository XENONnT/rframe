...
st.register_type_strategy(int, st.integers(min_value=-MAX_INTEGER, max_value=MAX_INTEGER))
...
datetimes_strategy = st.datetimes(min_value=MIN_DATETIME,
                         max_value=MAX_DATETIME,
                         allow_imaginary=False).map(round_datetime)

timedeltas_strategy = st.timedeltas(min_value=MIN_TIMEDELTA, 
...
int_left_strategy = st.integers(min_value=MIN_INTEGER, 
                                max_value=MAX_INTEGER)

int_length_strategy = st.integers(min_value=MIN_INTEGER + MIN_INTEGER_DELTA*2,
                                  max_value=MAX_INTEGER//100)

# Register the custom strategies for the two types of intervals TimeInterval, IntegerInterval.

st.register_type_strategy(IntegerInterval, make_interval(IntegerInterval,
                                                         int_left_strategy,
                                                         int_length_strategy))

interval_timedeltas_strategy = st.timedeltas(min_value=MIN_TIMEDELTA*2, 
                                    max_value=MAX_TIMEDELTA,)

st.register_type_strategy(TimeInterval, 
