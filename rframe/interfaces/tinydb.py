...
try:
    ...
    class TinyDBSelection:
        ...
        def apply(self, db):
            ...
            if self.sort is not None:
                docs = sorted(docs,
                        key=lambda x: tuple(x[s] for s in self.sort))
    ...
    class TinyDBInterface(DatasourceInterface):
        ...
        def get_simple_query(self, index, label, others=None):
            logger.debug('Building tinydb simple-query for index: '
        ...
        def interpolating_query(self, index, label):
            logger.debug('Building tinydb interpolating-query for index: '
                        f'{index} with label: {label}')
            name = index.name
            target = where(name)
            ...
            labels = {index.name: label}
            return TinyDBQuery(index, labels, self.source,
